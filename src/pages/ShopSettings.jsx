...
export default function ShopSettings() {
  ...
  async function deleteAllData() {
    if (!window.confirm("Delete all shop data permanently? This will remove all products, sales, customers, employees, salaries, expenses, drafts, and settings.")) return;
    if (!window.confirm("This cannot be undone. Continue deleting everything?")) return;
    try {
      await desktop?.deleteAll();
      setMessage("All shop data was deleted. Reloading the application…");
      window.setTimeout(() => window.location.reload(), 250);
    } catch (error) {
      setMessage(error?.message || "Could not delete shop data.");
    }
  }
